    ...
    <div className="relative space-y-8 pb-32">
      ...
      <motion.div custom={0} variants={fadeUp} initial="hidden" animate="visible" className="space-y-3">
        <div className="flex items-center gap-2 text-primary/50">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.2em]">{format(new Date(), 'EEEE, MMMM d')}</span>
        </div>
        ...
        <p className="text-sm font-medium text-muted-foreground/80 max-w-md">
          {isTodayComplete
            ? "Today's readings are complete — great work! 🌟"
            : todayPassages.length > 0
            ? `You have ${todayPassages.length - todayDone} of ${todayPassages.length} reading${todayPassages.length !== 1 ? 's' : ''} left to finish today.`
            : "No readings are scheduled for today. Rest and reflect."}
        </p>
      </motion.div>
      ...
      <motion.section custom={1} variants={fadeUp} initial="hidden" animate="visible"
        className={cn(
          "relative p-6 md:p-8 rounded-3xl border shadow-lg overflow-hidden transition-all duration-500",
        ...
        <div className="relative">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground/70">Daily Bread</p>
                <h2 className="text-base font-bold">{t.todaysReading || "Today's Reading"}</h2>
              </div>
            </div>
      ...
      <motion.div custom={2} variants={fadeUp} initial="hidden" animate="visible" className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <button key={card.label} onClick={card.onClick}
            className={cn("flex flex-col items-start gap-3 p-4 rounded-2xl border text-left transition-all hover:shadow-md hover:scale-[1.02] active:scale-[0.98] group relative", card.bg)}>
            <div className={cn("p-2 rounded-xl bg-background/60 border border-white/5 shadow-sm", card.color)}>
              <card.icon className="h-4 w-4" />
            </div>
            <div className="min-w-0 w-full">
              <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 truncate">{card.label}</p>
              <p className={cn("text-xl font-bold leading-tight mt-0.5", card.color)}>{card.value}</p>
      ...
      {unreadAlerts.length > 0 && (
        <motion.section custom={3} variants={fadeUp} initial="hidden" animate="visible" className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold uppercase tracking-wider text-orange-500">Alerts</h2>
      ...
      {upcomingItems.length > 0 && (
        <motion.section custom={4} variants={fadeUp} initial="hidden" animate="visible" className="space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Community Calendar</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {upcomingItems.map(item => (
              <button key={item.id} onClick={() => setSelectedEvent(item)}
                className="flex items-center gap-4 p-4 rounded-2xl border border-border/40 bg-card/40 backdrop-blur-sm hover:bg-card hover:shadow-md transition-all text-left group">
                <div className={cn("w-12 h-12 shrink-0 rounded-xl flex flex-col items-center justify-center border text-center", typeBg(item.type))}>
                  <span className={cn("text-[9px] font-bold uppercase", typeColor(item.type))}>{format(item.date, 'MMM')}</span>
                  <span className={cn("text-lg font-black leading-none", typeColor(item.type))}>{format(item.date, 'd')}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-sm truncate">{item.title}</p>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 mt-0.5">{typeLabel(item)} · {format(item.date, 'EEE')}</p>
                </div>
                <ChevronRight className="h-3.5 w-3.5 text-muted-foreground/20 group-hover:text-muted-foreground shrink-0 transition-colors" />
              </button>
            ))}
          </div>
        </motion.section>
