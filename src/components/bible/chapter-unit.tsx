    ...
    <div className="flex flex-col items-center gap-2">
      <Button
        ...
        className={cn(
          ...
          isCompleted 
            ? "bg-green-500 text-green-50 border-2 border-green-600 hover:bg-green-600 shadow-lg" 
      ...
      >
        ...
        <span className="relative z-10 text-lg font-bold">
            {isCompleted ? <Check className="w-6 h-6" /> : chapterNumber}
        </span>
      </Button>
      <div 
        className={cn(
            "w-3 h-3 rounded-full border-2",
            isCompleted ? "bg-green-500 border-green-600" : "bg-muted border-border"
        )}
      />
    </div>
  );
