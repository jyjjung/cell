...
export default function ChatList({
  ...
  return (
    <PageShell className="stack-gap-sm pb-20">
      ...
      <Tabs defaultValue="chats" className="relative">
        <TabsList aria-label="Chat categories">
          <TabsTrigger value="chats">Chats</TabsTrigger>
          {appScope === 'cell' && (
            <TabsTrigger value="birthday">
              Birthday archive{archivedChats.length > 0 ? ` (${archivedChats.length})` : ''}
            </TabsTrigger>
          )}
        </TabsList>
            ...
            <div className="overflow-hidden border-y border-border bg-background">
              <AnimatePresence mode="popLayout">
                {activeChats.map((chat, i) => {
                  ...
                  const isLast = i === activeChats.length - 1;
        ...
        <TabsContent value="birthday">
          {loading ? (
            ...
            <EmptyState
              icon={MessageCircle}
              title="No birthday memories yet"
              description="Archived birthday chats will appear here for everyone."
            />
          ) : (
            <div className="overflow-hidden border-y border-border bg-background">
              <AnimatePresence mode="popLayout">
                {archivedChats.map((chat, i) => {
                  ...
                  const lastSenderProfile = allUsers.find(u => u.uid === chat.lastMessageSenderId);
                  const lastSenderName = formatUserDisplayName(lastSenderProfile);
                  const isLast = i === archivedChats.length - 1;
                  return (
                    <motion.div key={chat.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="w-full">
                      <Link
                        href={href}
                        onClick={() => handleLinkClick(href)}
                        className={cn(
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 text-left">
                          <p className="truncate text-sm font-medium text-foreground">{details.name}</p>
                          <p className="truncate text-xs text-muted-foreground">
                            {chat.lastMessageText ? `${lastSenderName}: ${chat.lastMessageText}` : 'Archived birthday chat'}
                          </p>
                        </div>
                      </Link>
                    </motion.div>
