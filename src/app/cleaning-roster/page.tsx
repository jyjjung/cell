...
export default function CleaningRosterPage() {
    ...
    const renderEntry = (entry: CleaningRosterEntry, opts?: { scrollId?: boolean }) => {
        ...
        return (
            <div key={entry.id} id={opts?.scrollId ? `date-${entry.date}` : undefined} className="scroll-mt-20">
                <RosterFeedCard 
                    index={currentIndex}
                    date={entryDate}
                    ...
                    description={(
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                            ...
                            {assignedUsers.length > 0 && (
                                <span>
                                    {assignedUsers.map((user) => formatUserDisplayName(user)).join(', ')}
                                </span>
                            )}
                        </div>
                    )}
                            ...
                            {upcomingByMonth.length > 0 ? (
                                upcomingByMonth.map(([month, entries]) => (
                                    <RosterMonthGroup key={`up-${month}`} month={month}>
                                        {entries.map((entry) => renderEntry(entry, { scrollId: true }))}
                                    </RosterMonthGroup>
                                ))
                            ) : (
                                <EmptyState 
                                    ...
                                    return pastByMonth.map(([month, entries]) => {
                                        ...
                                        return (
                                            <RosterMonthGroup key={`past-${month}`} month={month}>
                                                {slice.map((entry) => renderEntry(entry))}
                                            </RosterMonthGroup>
                                        );
                                    });
                                })()
