/**
 * Remove a user (by email) from every chat's members list.
 * Usage: node scripts/remove-user-from-all-chats.mjs <email>
 */
...
if (!email) {
  console.error('Usage: node scripts/remove-user-from-all-chats.mjs <email>');
  process.exit(1);
}
...
async function run() {
  ...
  for (const chatDoc of chatsSnap.docs) {
    ...
    batch.update(chatDoc.ref, {
      ...
      [`memberInfo.${user.uid}`]: admin.firestore.FieldValue.delete(),
      [`memberSeen.${user.uid}`]: admin.firestore.FieldValue.delete(),
    });
    removed++;
    batchCount++;
  ...
  console.log(`Done. Removed from ${removed} chat(s), not a member of ${skipped}. Total chats: ${chatsSnap.size}`);
}
